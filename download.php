...
// Check if it's a download request or a download page request
if (isset($_GET['id']) && isset($_GET['file'])) {
    // This is a download page request
    $fileId = $_GET['id'];
    $fileName = $_GET['file'];
    
    // Query the database to get the file details
    $stmt = $conn->prepare("SELECT * FROM files WHERE id = ? AND file_name = ?");
    $stmt->bind_param("ss", $fileId, $fileName);
    $stmt->execute();
...
function displayDownloadPage($file_info) {
    $fileId = $file_info['id'];
    $fileName = $file_info['file_name'];
    $fileSize = $file_info['file_size'];
    $downloadCount = $file_info['download_count'];

    // HTML for the download page
    ?>
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <title>Download <?php echo htmlspecialchars($fileName); ?> - FilesWith</title>
    ...
    <body class="bg-gray-100 min-h-screen flex flex-col">
        ...
        <footer class="bg-gray-800 text-white py-4">
            <div class="container mx-auto text-center">
                <p>&copy; <?php echo date('Y'); ?> FilesWith. All rights reserved.</p>
            </div>
        </footer>
    </body>
    </html>
...
function processDownload($file_info) {
    $fileName = $file_info['file_name'];
    $fileId = $file_info['id'];
    $fileSize = $file_info['file_size'];
    $downloadCount = $file_info['download_count'];
    ...
    // Get the file path (adjust this according to your file storage system)
